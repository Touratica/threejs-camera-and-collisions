...
export default class Cue extends Component {
  ...
  unselected = new THREE.MeshBasicMaterial({
    color: 0xcc9966,
    wireframe: true,
  });
  ...
  constructor(
    ...
    this.cue_mesh = this.addCylinderHorizontal(
      this.unselected,
      cue_x,
      cue_y,
      0,
  ...
  shoot_ball() {
    ...
    this.shooted_ball = new Ball(
      this.steady_ball.radius,
      this.pool,
      this.steady_ball.position.x,
      this.steady_ball.position.y,
      new THREE.MeshBasicMaterial({ color: "white" }),
      new THREE.Vector3(vector.x * 50, vector.y * 50, 0)
    );
