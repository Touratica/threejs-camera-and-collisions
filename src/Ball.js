...
export default class Ball extends Component {
  ...
  // Gets distance between two balls
  ...
  // Moves the ball according to its velocity and the time since it was last called
  move(timeDelta) {
    ...
    if (!this.isFalling) {
      this.poolTable.holes.forEach((hole) => {
        ...
        if (
          Math.abs(this.position.x - centerX) < this.radius * 1.5 &&
          Math.abs(this.position.y - centerY) < this.radius * 1.5
        ) {
          this.velocity.z -= 10;
      ...
      let distance =
        this.poolTable.innerDepth / 2 -
        (Math.abs(this.position.x) + this.radius);

      //checks if the ball has colided the wall
      if (
        distance <= Math.abs(velocityX) &&
        Math.sign(this.position.x) === Math.sign(velocityX)
      ) {
        hasCollided = true;
        ...
        // hit the ball and returned back
        this.position.x =
          Math.sign(this.position.x) *
          (this.poolTable.innerDepth / 2 -
            this.radius -
            (velocityX - distance) * this.poolTable.wallCOR);
      ...
      distance =
        this.poolTable.innerWidth / 2 -
        (Math.abs(this.position.y) + this.radius);
      // Checks if the ball has colided the wall
      if (
        distance <= Math.abs(velocityY) &&
        Math.sign(this.position.y) === Math.sign(velocityY)
      ) {
        ...
        this.position.y =
          ...
          (this.poolTable.innerWidth / 2 -
            this.radius -
            (velocityY - distance) * this.poolTable.wallCOR);
  ...
  resolveCollision(ball, other) {
    if (ball.position.z !== this.position.z) {
      return;
    }
    ...
    if (dot_product >= 0) {
      ...
      let finalVelocity_Ball = {
        x:
          (initialVelocity_Ball.x * (mBall - mOther)) / (mBall + mOther) +
      ...
      let finalVelocity_Other = {
        x:
          (initialVelocity_Other.x * (mBall - mOther)) / (mBall + mOther) +
          (initialVelocity_Ball.x * 2 * mBall) / (mBall + mOther), //fixed here m1, instead of m2
  ...
  update(timeDelta, balls, size) {
    ...
    for (let i = 0; i < size; i++) {
      ...
      if (
        this.distanceToBall(balls[i].position.x, balls[i].position.y) <=
        this.radius * 2
      ) {
        // The two balls have colided
